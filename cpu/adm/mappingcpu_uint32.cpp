// 编译： g++ -std=c++17 -mavx512f -fopenmp -march=native -O3 mappingcpu_uint32.cpp -o mappingcpu_uint32 
// 执行： ./mappingcppV2_uint32 ../../../data/kron/kron-g500-logn19_43562265.u4 ../../../data/kron/test.bin
#include <iostream>
#include <fstream>
#include <vector>
#include <cstdint>
...
const int max_bytes_signal_per_ele_32b = 3;
...
bool load_file(const std::string& filename, std::vector<uint32_t>& data) {
...
int main(int argc, char** argv) {
    ...
    std::vector<uint32_t> centers(gsize);
    ...
    printf("compress cost %.2f ms, throughput %.2f MB/s\n", exe_time, num_elements * 4 / 1024 / 1024 / (exe_time / 1000));

    // 合并为连续数据结构
    size_t len1 = output_lengths.size() * sizeof(int);
    size_t len2 = centers.size() * sizeof(uint32_t);
    ...
    exe_time = 99999;
    std::vector<uint32_t> recovered(num_elements);
    for(int i = 0; i < times; i++)
    {
        auto start = std::chrono::high_resolution_clock::now();
    ...
    printf("CR : %.2f x\n",num_elements * sizeof(uint32_t) * 1.0 / total_size);
