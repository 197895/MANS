...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) {
            std::printf("[mans-original-write] %s\n", msg);
        }
    };
    ...
    std::snprintf(out, sizeof(out), "datasets/rank%d.mans-original.h5", rank);
    ...
    std::rewind(fp);
    if (bytes_l <= 0 || (bytes_l % static_cast<long>(sizeof(std::uint16_t))) != 0) {
        std::fprintf(stderr, "bad input size: %s\n", in);
        std::fclose(fp);
        MPI_Finalize();
        return 1;
    }
    std::size_t bytes = static_cast<std::size_t>(bytes_l);
    std::size_t elems = bytes / sizeof(std::uint16_t);
    ...
    mans::MansParams p{};
    p.backend = mans::Backend::CPU;
    p.dtype = mans::DataType::U16;
    p.adm_threshold = 4000;
    p.adm_decide_threads = 16;
    p.adm_center_calc_threads = 32;
    p.adm_encode_threads = 32;
    p.adm_warp_reduce_threads = 32;
    p.adm_fill_tail_threads = 16;
    p.adm_write_back_threads = 16;
    p.adm_restore_signals_threads = 32;
    p.adm_decode_values_threads = 16;
    ...
    if (rank == 0) {
        std::printf("[mans-original-write] chunk-size-mb=%.6g\n", chunk_size_mb);
    }

    hsize_t dims[1] = {static_cast<hsize_t>(elems)};
    hsize_t chunk[1] = {chunk_bytes / static_cast<hsize_t>(sizeof(std::uint16_t))};
    if (chunk[0] == 0 || chunk[0] > dims[0]) {
        chunk[0] = dims[0];
    }
    ...
    double sec = t1 - t0;
    double max_sec = 0.0;
    double local_bytes = static_cast<double>(bytes);
    double total_bytes = 0.0;
    double local_file_bytes = 0.0;
    ...
    if (ofp) {
        ...
        if (out_bytes_l > 0) {
            local_file_bytes = static_cast<double>(out_bytes_l);
        }
        std::fclose(ofp);
    }
    ...
    if (rank == 0) {
        ...
        double per_rank_approx = (nprocs > 0) ? (bw / static_cast<double>(nprocs)) : 0.0;
        double comp_ratio_x = (total_file_bytes > 0.0) ? (total_bytes / total_file_bytes) : 0.0;
        std::printf(
            "mans-original-write ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s comp_ratio=%.2fx\n",
