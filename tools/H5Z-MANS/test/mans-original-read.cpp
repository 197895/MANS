...
int main(int argc, char** argv) {
    ...
    auto stage = [&](const char* msg) {
        if (rank == 0) {
            std::printf("[mans-original-read] %s\n", msg);
        }
    };

    stage("opening input hdf5");
    char in[64];
    std::snprintf(in, sizeof(in), "datasets/rank%d.mans-original.h5", rank);
    ...
    std::size_t elems = static_cast<std::size_t>(dims[0]);
    std::size_t bytes = elems * sizeof(std::uint16_t);
    ...
    if (rank == 0) {
        ...
        std::printf(
            "mans-original-read ranks=%d total=%.1f MiB time=%.4f s throughput=%.2f MiB/s per_rank~%.2f MiB/s\n",
